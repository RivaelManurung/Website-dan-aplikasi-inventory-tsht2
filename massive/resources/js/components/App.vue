...
const getNotifications = async () => {
  ...
  loading.value = true;
  try {
    const { data } = await axios.get('http://192.168.172.64:8000/api/notifikasis?limit=15', {
...
const markRead = async (id) => {
  ...
  try {
    await axios.put(`http://192.168.172.64:8000/api/notifikasis/${id}/read`, {}, {
      headers: headers.value,
    });
  } catch (e) {
...
const markAllRead = async () => {
  ...
  try {
    await axios.put('http://192.168.172.64:8000/api/notifikasi/read-all', {}, {
      headers: headers.value,
    });
